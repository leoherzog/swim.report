...
//
// A live refresh (detailRefreshScript.js) rebuilds the wave section with a
// fresh, unlabelled row and dispatches "swimreport:refreshed"; the pass re-runs
// on it and skips every row it already relabelled, so a note is never doubled.
...
const SCRIPT_LINES = [
  ...
  "      const row = rows[i];",
  "      if (row.hasAttribute('data-relabeled')) {",
  "        continue;",
  "      }",
  "      row.setAttribute('data-relabeled', '');",
  ...
  "  };",
  "  relabel();",
  "  document.addEventListener('swimreport:refreshed', relabel);",
  "})();"
];

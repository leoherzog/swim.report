// src/wqFloor/chautauquaCountyNy.js — a raise-only water-quality floor source;
// see src/wqFloor/index.js for the floor contract.
//
// Source: Chautauqua County (NY) Health Department beach monitoring status page,
...
// src/wqFloor/index.js, re-resolve the live URL and set the constant.
//
// The page is assumed to be a server-rendered status listing, one entry per
// beach naming the beach and a status phrase. Because the markup could not be
// confirmed, parseChautauquaBeachStatus() depends on no tag structure: it strips
// HTML to plain text, locates each curated beach name, and reads the status
// phrase immediately following it. That tolerates table, list and card markup
// alike, at the cost of being defeated by a beach name with no nearby status
// text — which degrades to no site for that beach, never a guessed color.
//

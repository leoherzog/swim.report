...
Implement the scraper contract from PLAN.md (`matches(beach)` plus `scrape()` returning `{color, official: true, source, updated}`, or `null` on any failure) in `src/officialSources/`, and register it in the `scrapers` array in `src/officialSources/index.js`. A scraper may also declare the optional `staleMs`, `readingNote` and `officialTtlSeconds` fields. Parse defensively: a markup change upstream must degrade to `null`, never a wrong color. README has the full how-to.

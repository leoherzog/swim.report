...
files under `public/`, served by Workers static assets before the Worker runs, with the same
security headers the Worker sets applied from the hand-written `public/_headers`. No Worker
...
Every response also carries `strict-transport-security: max-age=31536000`,
...
an explicit per-route policy: successful API, beach-detail, `?ids=` and `/api/beaches.geojson`
responses send `cache-control: public, max-age=60, stale-while-revalidate=60,
stale-if-error=600`, so a cached flag is at most two minutes old; `/api/beaches.geojson` also
carries a strong SHA-256 `ETag` (Cloudflare presents it weak, `W/`, after compression) and
answers a matching `If-None-Match` with `304`; the
...
route, Workers Logs observability (`head_sampling_rate = 1`, query strings redacted from
logged URLs) and Smart Placement, all configured in `wrangler.toml`, which carries the real
production D1 and KV IDs (PLAN.md section 8 is authoritative). workers.dev and Preview URLs
are off, and each deploy uploads source maps.
...
A deploy also uploads `public/` ( `[assets] directory` in `wrangler.toml`): the hand-written
`_headers` rule that gives the platform-served files the Worker's security headers, and the
committed brand assets: favicon, apple-touch icon, the two manifest icons,
`manifest.webmanifest` and the five share cards. The brand assets are regenerated by
`node scripts/build-brand-assets.js`, which is idempotent, so re-run it and commit the diff
whenever the mark or the theme colors change; it never touches `_headers`.

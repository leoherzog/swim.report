...
`--marine-zones data/marine-zones-greatlakes.json` to `discovery-batch.js`. NWS already
...
second snapshot. The 11-column width matters: this is the only delete-bearing run, and its
truncation guard aborts the whole pass if D1's `--json` response is size-capped, so a paginated
snapshot is a recorded prerequisite for the North America expansion (see `TODO.md`).
...
zone per line, for a readable diff. Only Great Lakes, St. Lawrence and St. Clair zones are
kept, by id prefix: `GREAT_LAKES_ZONE_PREFIXES = ["LCZ", "LEZ", "LHZ", "LMZ", "LOZ", "LSZ",
"SLZ"]`. **This list must grow when `src/regions.js` `REGIONS` gains coasts beyond the Great
Lakes system.** Polygons are simplified at a `0.001`-deg tolerance, about 110 m of maximum
displacement, negligible against both the 15 km cap and the ~5 NM zone widths.
